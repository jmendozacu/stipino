 ...
 <ul class="breadcrumb">
    ...
    <li class="active"><a href="{{ URL::route('ProductsIndex') }}">Pregled svih proizvoda</a></li>
    
    <a href="{{ URL::route('ProductsCreate') }}" class="pull-right" style="margin-top: -5px;">
...
<div class="panel-heading">
    <h4>Pregled svih proizvoda ({{$productscount}})</h4>
</div>
        ...
        <tbody>
             @if (count($entries) > 0) 
                ...
                <tr>
                    ...
                    <td><a href="{{ URL::route('ProductsEdit', array('id' => $entry->id)) }}">{{ $entry->title }}</a></td>
                    <td>{{ $entry->joinproduct_weight }} {{ $entry->measure_unit }}</td>
                    <td>{{ $entry->description }}</td>
                    <td>{{ $entry->price }} kn</td>
                    <td>{{ $entry->manufacturing_price }} kn</td>
                    <td>@if($entry->onstock == '1')Da @else Ne @endif</td>
                    <td class="col-md-1">  
